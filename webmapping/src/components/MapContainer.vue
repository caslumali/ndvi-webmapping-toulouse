...
export default {
  ...
  data() {
    return {
      ...
      // BD Forêt avec style par défaut
      bdForetLayerName: "lima:Sample_BD_foret_3857",
      bdForetLayer: null,
      showBdForet: true,

      // BD Forêt avec le style par peuplement
      bdForetLayerStyledName: "lima:Sample_BD_foret_3857",
      bdForetStyledLayer: null,
      showBdForetStyled: false, // Initialement masqué
  ...
  methods: {
    ...
    */
    ...
    /*
      BD Forêt avec style par peuplement
    */
    initBdForetStyled() {
      this.bdForetStyledLayer = L.tileLayer.wms(this.geoserverUrl, {
        layers: this.bdForetLayerStyledName,
        styles: "lima:Sample_BD_Foret_essences_style", // Spécification du style
        format: "image/png",
        transparent: true,
      });
      if (this.showBdForetStyled) {
        this.bdForetStyledLayer.addTo(this.map);
      }
    },
    ...
    */
    changeBaseLayer(layerKey) {
      ...
      // Ré-ajouter BD Forêt (par peuplement) si actif
      if (this.showBdForetStyled && this.bdForetStyledLayer) {
        this.map.removeLayer(this.bdForetStyledLayer); // Supprime temporairement la couche stylisée
        this.bdForetStyledLayer.addTo(this.map);        // La ré-ajoute pour la placer au-dessus
      }
    ...
    /*
      BD Forêt (style par défaut) ON/OFF
    */
    toggleBdForet(show) {
      this.showBdForet = show;
      if (show) {
        this.map.removeLayer(this.bdForetStyledLayer); // Retire la couche stylisée si active
        this.bdForetLayer.addTo(this.map); // Ajoute la couche par défaut
      } else {
        this.map.removeLayer(this.bdForetLayer); // Retire la couche par défaut
      }
    },

    /*
      BD Forêt (style peuplement) ON/OFF
    */
    toggleBdForetStyled(show) {
      this.showBdForetStyled = show;
      if (show) {
        this.map.removeLayer(this.bdForetLayer); // Retire la couche par défaut si active
        this.bdForetStyledLayer.addTo(this.map); // Ajoute la couche stylisée
      } else {
        this.map.removeLayer(this.bdForetStyledLayer); // Retire la couche stylisée
      }
    },
